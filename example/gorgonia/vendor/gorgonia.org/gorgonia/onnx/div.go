...
// HadamardDiv2 performs a pointwise add.
type HadamardDiv2 struct{}

// Constructor to fulfil the interface ...
func (a *HadamardDiv2) Constructor() func(g graph.WeightedDirected, n graph.Node) (interface{}, error) {
	return func(g graph.WeightedDirected, n graph.Node) (interface{}, error) {
		...
		case len(x.Shape()) <= 1 && len(y.Shape()) != 1:
			...
			dims := make([]int, len(y.Shape()))
			for i := 0; i < len(y.Shape()); i++ {
				if len(x.Shape()) == 0 || y.Shape()[i] != x.Shape()[0] {
					dims[i] = 1
		...
		return engine.NewHadamardDivOperation(leftPattern, rightPattern)(g, n.(*engine.Node))
	}
}
